...
model_dict = {
    ...
    'ddensenet201': densenet201_finetune,
...
def train(epochNum, model_name='resnet18'):
    ...
    model = model_dict['resnet18'](num_classes=NB_CLASS).cuda()
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[20, 30, 40], gamma=0.1)
    for epoch in range(epochNum):
        ...
        #     optimizer = torch.optim.Adam(model.fresh_params(), lr=1e-3, amsgrad=True, weight_decay=1e-4)
        #     scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[20, 30, 40], gamma=0.1)
        # else:
        ...
        if accuracy > min_acc:
            ...
            torch.save({'epoch': epoch + 1,
                        ...
                        'optimizer': optimizer.state_dict(),
                        'val_loss': log_loss,
                        'val_correct': accuracy}, 'save_model/' + model_name + '/acc_beat/' + date + '_acc_best.pth')
            min_acc = accuracy
