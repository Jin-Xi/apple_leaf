...
def deleteNosiseType():
    ...
    description_val = open('../data/AgriculturalDisease_validationset/AgriculturalDisease_validation_annotations.json',
                           'r')
    ...
    with open("../data/AgriculturalDisease_trainingset/AgriculturalDisease_train_annotations_deleteNoise.json",
              'w') as f:
        json.dump(img_train, f, ensure_ascii=False)
    with open("../data/AgriculturalDisease_validationset/AgriculturalDisease_validation_annotations_deleteNoise.json",
              'w') as f:
...
def validateDataSplit(dataFrame, num=3):
    ...
    train = trainDataFrame.to_json(
        "../data/AgriculturalDisease_validationset/AgriculturalDisease_validation_annotations_train.json",
        orient='records', force_ascii=False)
    val = valDataFrame.to_json(
        "../data/AgriculturalDisease_validationset/AgriculturalDisease_validation_annotations_val.json",
        orient='records', force_ascii=False)


def split_dataset_pytorch(dataset, batch_size, validation_split=0.2, random_seed=42, num_workers=12, shuffle_dataset=True):
    ...
    train_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size,
                                               sampler=train_sampler, num_workers=int(num_workers * 0.8))
    validation_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size,
                                                    sampler=valid_sampler, num_workers=int(num_workers * 0.2))
...
def labelShuffling(dataFrame, outputPaht="../data/AgriculturalDisease_trainingset/",
                   outputName="AgriculturalDisease_train_Shuffling_annotations.json", groupByName='disease_class'):
